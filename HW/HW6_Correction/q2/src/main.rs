...
fn popular_recipes(
    ...
    // popularity counter holds refs until the very end
    ...
    for person in person_to_categories.keys() {
        for recipe in recipe_to_ingredients.keys() {
            if does_person_like_recipe(
                ...
                *recipe_popularity.entry(recipe).or_insert(0) += 1;
            }
        }
    }

    // sort by (likes DESC, name DESC) and take top 3
    ...
    ranked
        .into_iter()
        .take(3)
        .map(|(name, _)| name.clone())  // one clone per winner
        .collect()
}


    
    let mut sorted_recipes: Vec<_> = recipe_popularity.into_iter().collect();
    sorted_recipes.sort_by(|a, b| {
...
fn main() {
    let category_to_ingredients = load_category_ingredients("/Users/lanhakuken/Documents/GitHub/DS210_official_repo/HW/HW6/hw6_data/categories_ingredients.txt");
    let person_to_categories = load_people_categories("/Users/lanhakuken/Documents/GitHub/DS210_official_repo/HW/HW6/hw6_data/people_categories.txt");
    let recipe_to_ingredients = load_recipes("/Users/lanhakuken/Documents/GitHub/DS210_official_repo/HW/HW6/hw6_data/recipes.txt");
